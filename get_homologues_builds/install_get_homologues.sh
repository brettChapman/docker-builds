...
###///containerID=$(sudo docker run -v ${PWD}:${PWD} -it --name GETHOMOLOGUES -d local/get_homologues:latest)

####////sudo docker cp $containerID:/get_homologues .
...
###//sudo docker stop GETHOMOLOGUES
###///sudo docker rm $containerID

#Remove the local image of Get_homologues-est
####///imageID=$(docker images -a | sed -n '/local\/get_homologues/s/ \+/ /gp' | cut -d ' ' -f 3)
####///docker rmi $imageID

#Remove the original public Docker image of Get_homologues-est
####///imageID=$(docker images -a | sed -n '/csicunam\/get_homologues/s/ \+/ /gp' | cut -d ' ' -f 3)
#####////docker rmi $imageID
